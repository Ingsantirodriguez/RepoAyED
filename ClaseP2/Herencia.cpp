...
public:
   ...
   void incRadio(double); // incrementa el radio un cierto porcentaje
   double obtieneDiametro() const;       // devuelve el diametro
   double obtieneCircunferencia() const; // devuelve la circunferencia
   double obtieneArea() const;           // devuelve el area
   double getSemiCircPer() const;        // devuelve el perimetro de la circunferencia
...
void Circulo4::incRadio(double porcAumento)
{
    radio = (porcAumento > 0.0 ? radio*(1+porcAumento/100) : radio);
}
...
double Circulo4::getSemiCircPer() const
{
    return 3.1459 * obtieneDiametro()/2 + obtieneDiametro();
}
...
{
   cout << "Centro = ";
   Punto3::imprime();      // invoca a la funci�n imprime de Punto3, no de circulo
...
{  Circulo4 *circulo; // crea puntero a Circulo4
   ...
   circulo->incRadio(50);              // aumenta el radio en un 50%
