...
template <class T> class Pila:public Lista<T>{
      public:
             Pila(){Lista<T>();};
             void apilar(T d){this->add(d);};			// Agrega el elemento al comienzo/tope de la lista
             T tope(void){return this->cabeza();};		// Devuelve el tope de la lista	
             void desapilar(void){this->borrar();};		// Borra el tope de la lista 
             bool pilavacia(){return this->esvacia();};	// Devuelve true si la lista es vacia
};                  
...
int main()
{
  ...
  p->apilar("13");
  p->apilar("14");
  if (!p->pilavacia()) cout<<p->tope()<<endl;		// Imprime el tope de la lista (13)
  if (!p->pilavacia())p->desapilar();				// Desapila (elimina el 13)
  if (!p->pilavacia())cout<<p->tope()<<endl;		// Ahora el tope es 12
  if (!p->pilavacia())p->desapilar();				// Desapila (elimina el 12)
  if (!p->pilavacia())cout<<p->tope()<<endl;		// Ahora el tome es 11
  */
...
// que no se corresponda un cerrado con el ultimo abierto {  ]
  ...
  string cadena;
  cout<<endl<<"Ingrese cadena= "<<endl; 
  cin>>cadena;
  int error=0;
  for(int i=0;(i<cadena.length());i++){
	//	cout<<cadena.at(i)<<endl;			// El metodo at(i) devuelve el caracter en el indice i
		if (cadena.at(i)=='(' || cadena.at(i)=='[' || cadena.at(i)=='{'){
				  p->apilar(cadena.at(i));
  		}
		else if(cadena.at(i)==')' && p->tope()=='('){	// Cada uno de estos casos testea si
				p->desapilar();							// el caracter en el tope de la pila
		}												// es del mismo tipo, osea () o [] o {}
		else if(cadena.at(i)==']' && p->tope()=='['){
				p->desapilar();
		}
		else if(cadena.at(i)=='}' && p->tope()=='{'){
				p->desapilar();
		}
		else {											// En caso de no serlo, tira error
			error=1;
			i=cadena.length();
		}
  }
